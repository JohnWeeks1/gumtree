<template>
  <b-container id="edit-ad">
    <b-row>
      <b-col class="col-lg-8 offset-lg-2 col-md-8 offset-md-2">
        <b-form class="mt-3 form" @submit.prevent="update">
          <h3 class="pt-4 pb-2">Edit Ad</h3>
          ...
          >
            <b-row class="pl-2 pr-2">
              <b-col
                v-for="crop in cropped"
                :key="crop.image"
                class="col-3 p-1"
              >
                <b-img
                  v-b-modal.modal-center
                  :src="isBase64StringOrUrl(crop)"
          ...
          >
            <GoogleAutocomplete
              :formatted-address="location.placeResultData.formatted_address"
        ...
        <Modal @delete="deleteSingleImage()">
          <b-img
            :src="isBase64StringOrUrl(singleImageData)"
            class="w-100"
            center
            rounded
            alt="Thumbnail image"
...
export default {
  ...
  methods: {
    ...
    // Get ad by tha ad_id
    // Update current fields to needed data
    getAdById() {
      this.$store
        ...
        .then(() => {
          ...
          this.category = this.ad.category_id;
          this.getSubCategoryOptionsByCategoryId(this.ad.category_id);
          this.subCategory = this.ad.sub_category_id;
          let images = this.ad.images;
